...
func CreatePipeline(codecName string, tracks []*webrtc.TrackLocalStaticSample, pipelineSrc string) *Pipeline {
	...
	//pipelineStr = "rpicamsrc bitrate=256000 preview=false ! video/x-h264, width=640, height=480, framerate=30/1, stream-format=byte-stream, profile=baseline ! h264parse alignment=au, stream-format=byte-stream ! " + pipelineStr
	pipelineStr = "rpicamsrc bitrate=256000 ! video/x-h264 ! h264parse alignment=nal, stream-format=byte-stream ! " + pipelineStr
